...
Match taxonomic names against locally stored backbone databases, resolve synonyms, and enrich results with trait and status data. No API calls, no internet dependency, no rate limits. Nine backends, twelve enrichment layers, and a unified 16-column output schema.
...
taxize was removed from CRAN because its web API dependencies broke. WorldFlora works offline but only supports WFO, chokes on large batches, and offers no enrichment pipeline.

taxify provides offline matching against 9 backbone databases with fuzzy matching in C, synonym resolution, hybrid name handling, and a pipe-based enrichment system for joining trait and status data from 12 published datasets.
...
Twelve enrichment layers join trait and status data via backbone-resolved accepted names:

| Enrichment | Source | Taxa |
|------------|--------|------|
| `add_conservation_status()` | IUCN Red List | All |
| `add_invasive_status()` | GRIIS | All |
| `add_wcvp()` | WCVP | Plants |
| `add_woodiness()` | Zanne et al. | Plants |
| `add_eive()` | EIVE 1.0 | European plants |
| `add_diaz_traits()` | Diaz et al. | Plants |
| `add_leda()` | LEDA Traitbase | NW European plants |
| `add_elton_traits()` | EltonTraits 1.0 | Birds, mammals |
| `add_avonet()` | AVONET | Birds |
| `add_pantheria()` | PanTHERIA | Mammals |
| `add_amphibio()` | AmphiBIO | Amphibians |
| `add_common_names()` | GBIF | All |

Custom data can be joined via `add_data()`, which accepts data.frames, CSV, XLSX, SQLite, and .vtr files.
