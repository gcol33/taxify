...
taxify_download.taxify_ott <- function(backend, dest = NULL,
  ...
  write_backbone_meta(vtr_path, "ott", backend$version, .ott_url, nrow(df))
  write_version_meta(dest, "ott", backend$version, pinned = FALSE)
  ...
  if (verbose) {
    ...
    message(sprintf("OTT backbone saved: %s (%.0f MB, %s rows)",
...
#' @exportS3Method
taxify_load.taxify_ott <- function(backend, path = NULL, ...) {
  path <- path %||% file.path(taxify_data_dir(), "ott.vtr")
  if (!file.exists(path)) {
    stop(sprintf(
      "OTT backbone not found at: %s\nRun taxify_download('ott') first.",
      path
    ), call. = FALSE)
  }
  path
...
#' @exportS3Method
match_exact.taxify_ott <- function(backend, names_df, backbone, ...) {
  bb_path <- backbone
  n <- nrow(names_df)
  ...
  match_exact_compiled(result, names_df, bb_path, .ott_col_map)
}


#' @exportS3Method
match_fuzzy.taxify_ott <- function(backend, unmatched_df, backbone,
