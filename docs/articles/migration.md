...
For most of its lifespan, the R ecosystem had two main packages for
taxonomic name resolution: taxize for multi-database API lookups, and
WorldFlora for offline matching against the World Flora Online backbone.
Both served the community well, but both have reached the end of their
practical usefulness.

taxize was removed from CRAN in late 2023. The package queried a dozen
web APIs (GNRS, ITIS, NCBI, GBIF, Tropicos, and others) in real time,
which meant every call required internet access, was subject to rate
limits, and could break when an upstream endpoint changed its schema or
went offline. Results were not reproducible across sessions: the same
query could return different answers if a backbone was updated between
runs. The package’s core design made these problems structural, not
incidental. Even if someone maintains a fork, the API-first architecture
remains fragile.

WorldFlora is still on CRAN and still works. It reads the WFO
`classification.txt` file (a ~400 MB tab-separated download) and
performs string matching against it. The limitations are scope and
speed: it supports only WFO (plants), has no fuzzy matching beyond what
[`WFO.match.fuzzyjoin()`](https://rdrr.io/pkg/WorldFlora/man/WFO.match2.html)
provides via the fuzzyjoin package, and processes names by iterating
row-by-row through the backbone table. For datasets of a few hundred
names, the wait is tolerable. For datasets of ten thousand or more, it
...
(`.vtr` files powered by the vectra C11 engine), and runs all matching
offline. Nine backbones are available: WFO, COL, GBIF, ITIS, NCBI
Taxonomy, Open Tree of Life, WoRMS, Species Fungorum, and AlgaeBase.
Matching is vectorized at the C level with genus-blocked fuzzy joins, so
ten thousand names resolve in seconds rather than minutes. The output is
a standardized 16-column data.frame regardless of which backbone was
used. And because the backbone is a versioned file on disk, results are
