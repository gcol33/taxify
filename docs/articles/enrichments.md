...
the real analytical value emerges, and it is also where most workflows
hit friction: trait databases use different taxonomic authorities, store
names with or without authorship strings, treat synonyms inconsistently,
and distribute data in incompatible formats. Manually aligning names
...
locally. This build-from-source path is slower (it has to download and
parse raw data rather than a pre-built binary), but it means that
...
LEDA have ~8,000. These numbers include the cross-backbone name
expansion discussed earlier, so they are slightly larger than the
...
on the world’s major herbarium and botanical databases.
...
Seed mass and plant height sit on the two most important axes of the
global spectrum of plant form and function described by Diaz et
al. (2016, *Nature* 529:167-171). Seed mass captures the offspring size
/ offspring number trade-off (small-seeded species produce many
propagules, large-seeded species invest in fewer, better-provisioned
offspring). Plant height captures the competitive strategy axis (tall
species intercept more light but invest more in structural tissue).
Combining these two traits with EIVE or LEDA columns produces a
reasonably complete functional characterization for European temperate
species.
...
LEDA covers ~8,000 NW European plant species with 10 trait columns
spanning life form, dispersal, seed, leaf, and clonality dimensions. It
is the most column-rich of the simple enrichments, providing a broad
...
- `raunkiaer_life_form`: the primary Raunkiaer life form (phanerophyte,
  chamaephyte, hemicryptophyte, geophyte, therophyte, helophyte,
  hydrophyte).
...
- `terminal_velocity_ms`: seed terminal velocity in m/s (species
  median).
...
invertebrates, and fungi. A single column is added:
`conservation_status`, containing the standard IUCN category code.
...
- **NT** (Near Threatened): close to qualifying for a threatened
  category.
...
- **EW** (Extinct in the Wild): survives only in captivity or
  cultivation.
...
habitat breadth. It remains the most-cited source of mammalian
life-history data in the ecological literature, despite being published
in 2009.
...
The Least Weasel (*Mustela nivalis*) in the example above illustrates
the dynamic range of mammalian traits: at 67 g body mass and a home
range of 0.03 km^2, it sits at the opposite end of the spectrum from the
Brown Bear (*Ursus arctos*) at 139,000 g and 488 km^2. These allometric
scaling relationships (body mass predicting home range, longevity,
gestation, etc.) are a major reason PanTHERIA is so widely cited.
...
values in the enrichment (196 countries for GRIIS, dozens of TDWG
regions for WCVP).
...
or extinct in TDWG Level 2 botanical regions. TDWG (Taxonomic Databases
...
planted or naturalized. This is an important distinction: many
cultivated species will show `NA` in regions where they are widespread
in gardens and plantations.

Querying multiple regions reveals each species’ native continental
range:
...
code from the query does not mean the species lacks native range data;
it means we did not ask about the right region.

The full list of available TDWG codes can be retrieved programmatically
from the manifest (the `available_groups` field for the `wcvp`
...
each enrichment operates independently on the `accepted_name` column.
The only case where order matters is the column-name collision between
LEDA and Diaz (`seed_mass_mg`), discussed earlier.
...
(`avonet_body_mass_g` and `elton_body_mass_g`), so there is no
overwriting.
...
and LEDA (~8,000). However, for a specifically NW European dataset,
LEDA’s ~8,000 species may actually cover a larger fraction of the
species list than the Diaz dataset, because LEDA is geographically
focused on the same region.
...
4.  **Infraspecific taxa.** Most enrichments operate at the species
...
for the methods section of a paper. Rather than writing “we used the
...
The `seed_mass_mg` collision between LEDA and Diaz was discussed
...
publications. The [`summary()`](https://rdrr.io/r/base/summary.html)
output includes the source and version for each applied enrichment,
