...
Belgium who writes a slightly misspelled name meant a Belgian plant, and its
one-letter neighbour from New Zealand is a different species. The string
distance alone cannot tell the two apart; the geography can.
...
`region = "Belgium"` reach the same place. An unrecognised region or code
(`"GRE"` for Greece, which is `GRC`) is dropped with a warning rather than
failing the call, so the match runs unconstrained. A valid region with no WCVP
record for a candidate leaves that candidate unfiltered.
