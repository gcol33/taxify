...
of a formula (with their resolved accepted names and backbone ids) are added on
demand by `add_hybrid_info()`, covered below.
...
`matched_name` and `accepted_name` -- never collapsed to one parent. To work
with the parents, use `add_hybrid_info()`, which resolves both against the same
backbone:
...
parents from the `input_name` column and resolves each against the backbone(s)
