...
resolution. A **hybrid** names a cross
...
names the kind, regardless of whether the name matched. Both describe the input
itself, set during cleaning before any backbone match.
...
input names only the hybrid itself. The parent species of
...
"x" sits inside the word, with no surrounding whitespace to mark it as a hybrid.
...
microspecies open. Two markers signal one:
...
Both mean the same thing for matching: the name refers to the whole group of
microspecies. taxify folds the spelling variants a source might use
(`agg.`, `aggr.`, `agg`, `-agg`, `coll. sp.`, `sensu lato`) to a canonical
`agg.` or `s.l.` marker, recorded in the `qualifier` column. The opposite marker,
*s.str.* (*sensu stricto*, "in the narrow sense"), narrows a name to its core
binomial, so it is recorded in `qualifier` and matched as an ordinary name.
...
as a concept of its own; the others record the binomial alone, without the group
...
back to the nominal binomial. The result carries an `aggregate_fallback` column
recording which happened: `FALSE` when the dedicated aggregate taxon was found,
`TRUE` when it fell back to the binomial, and `NA` for non-aggregate names.
...
resolves an aggregate through the taxonomic hierarchy:
...
- an **aggregate** query takes the aggregate-level value first, and where the
  ...
  pragmatic stand-in: the species' own measurement stands in for the group where
  no aggregate-level figure exists.
