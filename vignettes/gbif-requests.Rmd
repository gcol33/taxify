...
returns the records with your names attached. The request and the fetch are
...
records with your queried name on every row:
...
closes it the same way. Matching first is only needed when you want to read the
matches before anything is sent. The matched table is still there afterwards, as
`attr(dl, "taxa")`.
