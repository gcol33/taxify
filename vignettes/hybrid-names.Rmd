...
(*Hesperotropsis*). The hybrid formula did **not** collapse to its first
parent: WFO carries no single record for this cross, so `match_type` is
`hybrid_formula`. Because both parents resolve against WFO, the cross is named
by them -- `accepted_name` is "Salix alba × Salix fragilis" (the accepted-parent
cross) and `matched_name` the input-parent cross. When a parent cannot be
matched, both stay `NA`. Either way the row is never a misleading single match
to *Salix alba*.
