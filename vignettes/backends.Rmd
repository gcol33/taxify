...
databases. Nine backends are available, each compiled from a different
...
The table below summarizes the nine backends. "Approx. names" is the total
...
authorship. Species Fungorum and AlgaeBase provide authorship from their
DwC-A archives.
...
just the numeric ID. Species Fungorum and AlgaeBase use ChecklistBank
...
has denormalized classification columns directly in its DwC-A. The genus
...
WFO, COL, and GBIF. The register contains ~100k genera, each with its family,
higher classification (kingdom through order, where available), and a
`life_form` label (e.g., `"vascular plant"`, `"animal"`, `"fungus"`). The
classification is resolved by priority: COL > GBIF > WFO. If COL and WFO
disagree about which family a genus belongs to, COL's assignment wins.
...
download time and does not silently update mid-session. Version checks happen
