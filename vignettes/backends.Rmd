...
databases. Fifteen backends are available, each compiled from a different
...
The table below summarizes the fifteen backends. "Approx. names" is the total
...
Every row in the output has 26 columns regardless of which backend produced
...
`ambiguous_targets`, `backend`, `backbone_version`, `kingdom_group`,
`taxon_group`, `life_form`, `qualifier`, `qualifier_position`,
`aggregate_fallback`, and `hybrid_type`. The `backend`
...
there (GBIF has ~6.4M names, the largest of any backend) and the curated
...
backend. It is curated specifically for fungi, with ~315k names including
...
26 columns are sufficient, and backend-specific extras are only needed when
