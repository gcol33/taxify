...
values should exist. The standard workaround is to run the external names
...
The function reads the first sheet by default. If the data spans multiple
sheets, the simplest approach is to read the target sheet into a data.frame
first using `openxlsx2::read_xlsx()` and pass that data.frame to
`add_data()`.
