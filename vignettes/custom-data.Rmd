...
The first sheet is read by default. Use the `sheet` argument to target a
different sheet by number or name:
...
As with CSV files, `species_col` is auto-detected when omitted: each
character column is tested against the backbone, and the column with the
highest match rate is selected. If no column reaches 50% matches, an error
asks you to specify `species_col` explicitly.
...
re-used in another without converting back through CSV.

## Other file formats

For formats not directly supported (`.tsv`, `.parquet`, `.rds`), reading the
