...
the same output on any machine. <!-- manifest:backbone-count -->19<!-- /manifest:backbone-count --> Darwin Core backbones are available
(WFO, COL, GBIF, ITIS, NCBI, OTT, WoRMS, Euro+Med, Species Fungorum, AlgaeBase,
FishBase, SeaLifeBase, Reptile Database, LCVP, WCVP, MDD, AviList, LPSN), all
queried offline.

The first `taxify()` call installs the default backbone set (COL, GBIF, and ITIS)
once, about 4 GB, and caches it under `taxify_data_dir()`. After that, nothing touches the network.
...
(`backbone = c("col", "gbif", "itis")`) are covered in the
