...
- **Multiple backbones.** taxify matches against <!-- manifest:backbone-count -->18<!-- /manifest:backbone-count --> backbones offline
...
- **Enrichments.** Results pipe directly into more than eighty published trait
...
| `children()` / `downstream()` | `children()` | Lists the accepted taxa within a genus or family |
...
taxify does not do" section below points to the right
...
vector with attributes). taxify returns the same 26-column data.frame
...
via `add_wfo_info()` when needed, but the default output is the same 26
columns whether the backbone is WFO, COL, or GBIF.
...
**Multi-backbone.** taxify supports <!-- manifest:backbone-count -->18<!-- /manifest:backbone-count --> backbones through a single
...
pinning is also available: `taxify_download("col", version =
...
- **Live API access to upstream databases.** taxize, rgbif, worrms, ritis,
  and TNRS query their backbones in real time. If you need a name added to
...
taxify bundles more than eighty enrichment datasets that cover conservation
status, invasive species, functional traits, morphological measurements, and
...
locally, following the same pattern as backbones. `list_enrichments()`
lists every source-named `add_*()` door, and the cross-source
`add_trait()` verb gathers a single trait across every source that
carries it.
