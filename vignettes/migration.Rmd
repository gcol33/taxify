...
exactly which snapshot was used. If your workflow already uses one of the other
...
4. **Enrich** the result with the `add_*()` layers that `list_enrichments()`
   lists, or join your own table with `add_data()`.
...
multiple matches, and you pick the row you want with `dplyr::filter`. taxify
...
- **Wider biodiversity-data cleaning.** [bdc](https://brunobrr.github.io/bdc/)
  ...
  taxonomic step alone if you prefer offline backbones over taxadb + GNR; the
...
- **Interactive, per-name resolution with manual disambiguation.** taxize had
  ...
  picks the best match automatically (accepted name over synonym, species rank
  over higher ranks, lowest ID as tiebreaker) and flags an unresolved homonym in
  `is_ambiguous`, with the candidates in `ambiguous_targets`. If manual control
  over ambiguous matches is needed, direct API calls may be preferable.
