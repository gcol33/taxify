...
performs string matching against it. The limitations are scope and speed:
it supports only WFO (plants), has no fuzzy matching beyond what
`WFO.match.fuzzyjoin()` provides via the fuzzyjoin package, and
processes names by iterating row-by-row through the backbone table. For
datasets of a few hundred names, the wait is tolerable. For datasets of
ten thousand or more, it becomes a bottleneck.
...
| `classification()` | `taxify()` | Returns `family`, `genus`, `rank` directly; `add_col_info()` for full hierarchy |
...
| `get_nativecountry()` | `add_wcvp()` | WCVP native range by TDWG region (plants) |
...
`classification()` returned a nested list of data.frames,
`synonyms()` returned another nested list. taxify returns the same
16-column data.frame from every call. Synonym status, taxonomic
hierarchy, and match quality are columns, not separate API calls.
...
still accessible via `add_wfo_info()` when needed, but the default
output is the same 16 columns whether the backend is WFO, COL, or GBIF.
...
`taxify()` call downloads and converts the backbone, subsequent calls
...
Each of those four steps makes network requests. If the GBIF API is
slow or rate-limiting, the whole pipeline stalls. The results come back
in different shapes: `resolved` is a data.frame, `gbif_ids` is a
character vector with class attributes, `class_list` is a list of
data.frames (one per name), and `syn_list` is another list of
data.frames.
...
`taxonomicStatus` to tell accepted names from synonyms, and cross-reference
`acceptedNameUsageID` against `taxonID` to find the accepted name string.

taxify folds exact matching, fuzzy matching, and synonym resolution
into a single call. Fuzzy matching is on by default and uses
...
`Quercus pedunculata`, and the synonym resolver maps it to `Quercus
robur` (the accepted name). WorldFlora requires the user to chain
`WFO.match.fuzzyjoin()` and then manually trace the
`acceptedNameUsageID`.
...
`add_data()` handles arbitrary external datasets: pass a CSV path,
an XLSX file, a SQLite database, or a plain data.frame, and taxify
matches the species names through the same backbone(s) before joining.


## Key differences at a glance

**Online vs. offline.** taxize sent HTTP requests for every name.
taxify downloads backbone files once and matches locally. After the
initial download (typically 50--300 MB depending on the backbone), no
internet connection is needed. Queries against the local backbone run in
...
WorldFlora's `WFO.match()` processes names sequentially against a
loaded data.frame. taxify uses vectra's C-level join engine with hash
indexes and genus-blocked fuzzy joins, processing thousands of names per
second on a single core. Fuzzy matching runs in parallel across four
threads by default.
...
"2024.06")` downloads a specific release that will never be
overwritten.
...
from "European robin" to *Erithacus rubecula*. taxify works in the
opposite direction: it matches scientific names and can attach common
names via `add_common_names()`, but it cannot start from a vernacular
name.
...
For research workflows where last-week freshness matters more than
...
- **Common-to-scientific lookups** (`comm2sci()`). If the starting point
  ...
  (`rgbif::name_suggest()`) accepts common names and returns scientific
  name candidates.
...
- **Interactive, per-name resolution with manual disambiguation.** taxize
  ...
  If manual control over ambiguous matches is needed, the taxize fork
  maintained on GitHub or direct API calls may be preferable.
...
into `taxify()` plus optional `add_*()` pipes. The output is a flat
data.frame, not nested lists. Matching runs offline against versioned
backbone files, so results do not change between sessions unless the
user explicitly updates the backbone.
...
(rotl, rgbif, spocc, rentrez) remain the right tools. taxify handles
the name-matching step that comes before all of those.
